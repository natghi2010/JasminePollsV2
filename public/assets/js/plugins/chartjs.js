...
(function (global, theme) {
  ...
  var init = function(){
    ...
    var d_b = {
        labels: Utils.months({count: 10, section: 3}),
        datasets: [
            {
                label: 'Sales',
                data: Utils.numbers({count: 10, decimals: 2}),
                fill: true,
...
function drawBarChart(id,labels,data,title){
  var ctx = document.getElementById('chart-bar-'+id);
...
axios.get($("#generalUrl").val()+'/graphApi/subcity').then(resp=>{
 drawBarChart('subcities',resp.data.key,resp.data.value,'Subcities')
});

axios.get($("#generalUrl").val()+'/graphApi/sector/city').then(resp=>{
  var key =  [];
  var value =  [];
  resp.data.map(sector=>{
    key.push(sector.key)
    value.push(sector.value)
  })
  drawBarChart('sectors',key,value,'Sectors')
});

axios.get($("#generalUrl").val()+'/graphApi/sector/subcity').then(resp=>{
  var key =  [];
  var value =  [];
  resp.data.map(sector=>{
    key.push(sector.name)
    value.push(sector.value)
  })
  drawBarChart('subcity-sectors',key,value,'Sectors')
});

axios.get($("#generalUrl").val()+'/graphApi/questions').then(resp=>{
  var key =  [];
  var value =  [];
  ...
  drawBarChart('questions',key,value,'Questions')
  });
    ...
    axios.get($("#generalUrl").val()+'/graphApi/sector/subcity').then(resp=>{
          ...
          resp.data.map(data=>{
            drawEasyPieGraph(data.key,data.value)
          })
    ...
    for(var i=0;i<$(".graphText").length;i++){
      ...
      var id = $(".graphText").eq(i).attr('id');
      var value = $(".graphText").eq(i).attr('title');
      drawEasyPieGraph(id,Number(value))
    }
    ...
    // axios.get($("#masterUrl").val()+'/graphApi/sector/subcity').then(resp=>{
...
//                       borderColor: ['#d0ce10', theme.color.warning],
//                       borderWidth: 6
//                   }
//               ]
//           }
...
//     var ctx = document.getElementById('chart-pie-5');
...
//       var ctx = document.getElementById('chart-pie-6');
...
//       var ctx = document.getElementById('chart-pie-7');
...
//       var ctx = document.getElementById('chart-pie-8');
...
//       var ctx = document.getElementById('chart-pie-9');
...
//       var ctx = document.getElementById('chart-pie-10');
...
//       var ctx = document.getElementById('chart-pie-11');
...
//        var ctx = document.getElementById('chart-pie-12');
...
//        var ctx = document.getElementById('chart-pie-13');
...
//  var ctx = document.getElementById('chart-pie-14');
...
//       );
  ...
  function drawEasyPieGraph(id,value){
    console.log('chart-pie-'+id)
    console.log('gettingvalue',value)
    var ctx = document.getElementById('chart-pie-'+id);
    if(value < 60){
      var statusColor = ['#e40f46',theme.color.danger]
    }else if(value >= 60 && value < 79){
      var statusColor = [theme.color.danger,theme.color.warning]
    }
    else if(value >= 79 && value < 95){
      var statusColor =  [theme.color.warning,theme.color.success]
    }
    else{
      var statusColor =  [theme.color.success,theme.color.info]
    }
